...
printf("%s choose to bowl\n",toss);
 ...
 gotocount++;
   ...
   {  if(bwl==10)
    ...
    gameover:
    FILE *outputFile;
outputFile = fopen("match_result.txt", "a");  // Open in append mode
...
int maxNameLength = 0;
  ...
  fprintf(outputFile, "\n===================================================================================================================\n");
fprintf(outputFile, "\n%s scores %d out of %d overs\n", team[choice1 - 1].name, team[choice1 - 1].totalrun, 2);
fprintf(outputFile, "Name\t\tscore\tFours\tSixes\t\n");
...
fclose(outputFile); 
  ...
  if(team[choice1-1].totalrun>team[choice2-1].totalrun)
    printf("%s won by %d runs",team[choice1-1].name,team[choice1-1].totalrun-team[choice2-1].totalrun);
  else 
...
void run()
{
   ...
   if(team[choice1-1].totalrun>target){
    printf("%s won ",team[choice1-1].name);
    flag=1;
   }
